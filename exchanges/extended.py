...
class ExtendedClient(BaseExchangeClient):
    ...
    async def place_maker_order(self, contract_id: str, quantity: Decimal, direction: str) -> OrderResult:
        ...
        This method is specifically designed for Maker mode trading where:
        - Orders are placed at BBO prices to ensure maker execution
        - Returns detailed order information for monitoring
        - Does not immediately retry on POST_ONLY rejection (patient waiting)
        ...
        Returns:
            OrderResult with detailed order information including:
            - order_id: Unique order identifier for monitoring
        ...
        Note:
            This method uses post_only=True to ensure Maker fees (~0%).
            The order should be monitored for fill status using WebSocket updates.
        """
        try:
            if self.orderbook is None:
                self.logger.log("Orderbook not ready, waiting...", level="INFO")
                await asyncio.sleep(1)
                return OrderResult(success=False, error_message='Orderbook not ready')
            ...
            if direction.lower() == 'buy':
                # For buy orders, place at best bid to ensure maker execution
                order_price = best_bid
                side = OrderSide.BUY
            else:
                # For sell orders, place at best ask to ensure maker execution
    ...
    def get_order_update_handler(self):
        """
        Get the order update handler for MakerOrderMonitor.

        Returns:
            The handle_account method that processes order updates via WebSocket.

        Note:
            This handler receives WebSocket messages for account/order updates.
            It updates self.open_orders dictionary with order status changes.
        """
        return self.handle_account

    def get_orderbook_handler(self):
        """
        Get the orderbook handler for PriceMonitor.

        Returns:
            The handle_orderbook method that processes orderbook updates via WebSocket.

        Note:
            This handler receives WebSocket messages for orderbook updates.
            It updates self.orderbook with latest BBO prices.
        """
        return self.handle_orderbook
    ...
    def get_current_orderbook(self) -> dict:
        ...
        Returns:
            Dictionary with current orderbook data:
            {
                'bid': [(price, quantity), ...],
                'ask': [(price, quantity), ...],
                'best_bid': Decimal,
            ...
            Returns None if orderbook is not yet available.
        """
        if self.orderbook is None:
            return None

        return {
            'bid': self.orderbook.get('bid', []),
            'ask': self.orderbook.get('ask', []),
            'best_bid': Decimal(self.orderbook['bid'][0]['p']) if self.orderbook.get('bid') else Decimal('0'),
            'best_ask': Decimal(self.orderbook['ask'][0]['p']) if self.orderbook.get('ask') else Decimal('0'),
        }

    def get_open_orders(self) -> dict:
        """
        Get current open orders dictionary.

        Returns:
            Dictionary of open orders: {order_id: order_info}
            Each order_info contains: id, market, side, qty, filledQty, price, status, etc.
        """

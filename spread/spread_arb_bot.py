...
class SpreadArbBot:
    ...
    def __init__(self, config: BotConfig):
        ...
        self._open_confirm_notify_ts: Optional[float] = None
        self._open_confirm_last_msg: Optional[str] = None
    ...
    def _notify_open_confirming(self, mode: str, reason: str) -> None:
        now = time.time()
        # 简单节流：5秒内不重复推相同原因
        if self._open_confirm_last_msg == reason and self._open_confirm_notify_ts and now - self._open_confirm_notify_ts < 5.0:
            return
        self._open_confirm_notify_ts = now
        self._open_confirm_last_msg = reason
        self._notify(
            "🕒 开仓确认中",
            [
                f"时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
    ...
    def _notify_rollback_in_progress(self, ext_position: Decimal, lig_position: Decimal, reduce_qty: Decimal) -> None:
        ...
        if self._open_rollback_error_by_exchange:
            err_parts = [f"{k}={v}" for k, v in self._open_rollback_error_by_exchange.items()]
            lines.append(f"回滚错误: {', '.join(err_parts)}")
        recent_logs = self._get_recent_log_tail(20)
        if recent_logs:
            lines.append("终端上下文(最近20行):")
            lines.extend(recent_logs)
